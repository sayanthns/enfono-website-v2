...
const AdminCMS = () => {
    ...
    const [sidebarGroup, setSidebarGroup] = useState('content');
    ...
    const sidebarItems = {
        content: [
            { id: 'leads', icon: 'fas fa-inbox', label: 'Leads' },
            { id: 'blogs', icon: 'fas fa-rss', label: 'Blog Posts' },
            { id: 'media', icon: 'fas fa-photo-video', label: 'Media & Events' },
            { id: 'work', icon: 'fas fa-briefcase', label: 'Our Work' },
            { id: 'about', icon: 'fas fa-building', label: 'About Page' },
        ],
        site: [
            { id: 'hero', icon: 'fas fa-home', label: 'Hero Section' },
            { id: 'services_hero', icon: 'fas fa-cogs', label: 'Services Hero' },
            { id: 'ai_cta', icon: 'fas fa-robot', label: 'AI Page CTA' },
            { id: 'stats', icon: 'fas fa-chart-bar', label: 'Stats' },
            { id: 'brands', icon: 'fas fa-cube', label: 'Products' },
            { id: 'careers', icon: 'fas fa-users', label: 'Careers' },
            { id: 'clients', icon: 'fas fa-handshake', label: 'Clients' },
            { id: 'testimonials', icon: 'fas fa-quote-right', label: 'Testimonials' },
        ],
        global: [
            { id: 'chatbot', icon: 'fas fa-comment-dots', label: 'AI Chatbot' },
            { id: 'file_manager', icon: 'fas fa-folder-open', label: 'File Manager' },
            { id: 'backup', icon: 'fas fa-download', label: 'Backup & Restore' },
        ]
    };

    const allTabs = [...sidebarItems.content, ...sidebarItems.site, ...sidebarItems.global];
    const activeLabel = allTabs.find(t => t.id === activeTab)?.label || '';

    const sidebarGroupStyle = (group) => ({
        padding: '6px 12px', fontSize: '10px', fontWeight: 700, textTransform: 'uppercase',
        letterSpacing: '1px', color: '#94a3b8', marginBottom: '4px', marginTop: '16px'
    });

    const navBtnStyle = (tab) => ({
        display: 'flex', alignItems: 'center', gap: '10px', width: '100%',
        padding: '10px 14px', background: activeTab === tab ? 'rgba(16,185,129,0.12)' : 'none',
        border: activeTab === tab ? '1px solid rgba(16,185,129,0.25)' : '1px solid transparent',
        borderRadius: '8px', color: activeTab === tab ? '#10B981' : '#475569',
        cursor: 'pointer', fontWeight: activeTab === tab ? 700 : 500, fontSize: '13px',
        textAlign: 'left', transition: 'all 0.15s'
    });

    return (
        <div className="admin-cms" style={{ display: 'flex', gap: '0', minHeight: '100vh', alignItems: 'flex-start' }}>
            {/* Sidebar */}
            <div style={{
                width: '220px', flexShrink: 0, background: '#fff', borderRight: '1px solid #e2e8f0',
                padding: '24px 12px', position: 'sticky', top: 0, minHeight: '100vh', overflowY: 'auto'
            }}>
                <div style={{ fontWeight: 800, fontSize: '15px', color: '#0f172a', padding: '0 4px', marginBottom: '4px' }}>Enfono CMS</div>
                <div style={{ fontSize: '11px', color: '#94a3b8', padding: '0 4px', marginBottom: '20px' }}>Admin Panel v2.1</div>

                <div style={sidebarGroupStyle()}>Content</div>
                {sidebarItems.content.map(tab => (
                    <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={navBtnStyle(tab.id)}>
                        <i className={tab.icon} style={{ width: '16px', textAlign: 'center' }} />
                        {tab.label}
                    </button>
                ))}

                <div style={sidebarGroupStyle()}>Site Settings</div>
                {sidebarItems.site.map(tab => (
                    <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={navBtnStyle(tab.id)}>
                        <i className={tab.icon} style={{ width: '16px', textAlign: 'center' }} />
                        {tab.label}
                    </button>
                ...
                <div style={sidebarGroupStyle()}>Global Settings</div>
                {sidebarItems.global.map(tab => (
                    <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={navBtnStyle(tab.id)}>
                        <i className={tab.icon} style={{ width: '16px', textAlign: 'center' }} />
                        {tab.label}
                    </button>
                ...
                <div style={{ marginTop: '24px', paddingTop: '16px', borderTop: '1px solid #e2e8f0' }}>
                    <a href="/" target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#10B981', fontSize: '13px', textDecoration: 'none', padding: '8px 14px' }}>
            ...
            <div style={{ flex: 1, padding: '32px', minWidth: 0 }}>
                <div className="admin-header" style={{ marginBottom: '24px' }}>
                    <h1 style={{ fontSize: '22px', fontWeight: 800, color: '#0f172a' }}>{activeLabel}</h1>
